...
# Use rminer instead of e1071 to get variable importance out
```{r}
# library(rminer)
# 
# svm_prob_fit <- fit(x = species_observed ~ ., data = train, model = "svm", task = "prob")
# svm_imp <- Importance(svm_prob_fit, data = train)
# summary(svm_prob_fit)
# svm_prob_fit
# svm_imp
# best_linear_svm <- svm(formula = species_observed ~ ., 
#                  data = train, 
...
# train_svm_predict <- predict(svm_prob_fit, newdata = select(train, -c(1)))
# train_svm_predict
# # svm_predict_prob <- predict(best_linear_svm, newdata = select(test, -c(1)),probability = TRUE, type = "response")
# # best_svm_predict$predictions[,2]
# # best_svm_predict.decision_function(newdata = select(test, -c(1)))
# test_svm_predict <- predict(svm_prob_fit, select(test, -species_observed))
# test_svm_predict
# 
# # train_cf <- table(train$species_observed, train_svm_predict)
# train_cf <- confusionMatrix(train$species_observed, train_svm_predict)
# train_cf
# 
# test_cf <- confusionMatrix(test$species_observed, test_svm_predict)
# test_cf
# 
# 
...
pred_er <- bind_cols(pred_surface_eff, encounter_rate = as.integer(pred_svm)) %>% 
